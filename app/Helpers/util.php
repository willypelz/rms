...
function qualifications()
{
    return $options = [
        ...
        'N.C.E' => 'N.C.E',
        'Diploma' => 'Diploma',
        'High School (S.S.C.E)' => 'High School (S.S.C.E)',
...
function remove_cv_contact($cv)
{
    ...
    $states             = array_merge($states,
        ["Avenue", "Close", "Estate", "District", "Post Office", "Crescent", "Street"]);
    $extaracted_content = $cv['extracted_content'][0];
    // remove email
    $extaracted_content = preg_replace('/([a-zA-Z0-9_\-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)/',
        '*****@****.***', $extaracted_content);

    // remove phone
    $extaracted_content = preg_replace("/((\+)*\s*\(*(234|0)\)*\s*\(*(234|0|1*)\)*\s*\-*\s*\d{3}\s*\-*\s*\d{3}\s*\d{4}|(\+)*\s*\(*(234|0|1)\)*\s*\(*(234|0|1)*\)*\s*\-*\s*\d{3}\s*\-*\s*\d{4})/i",
        '***-***-****', $extaracted_content);
    // '/([+]?[0-9]+[\- ]?[0-9]+)/'
    ...
    $extaracted_content = preg_replace('/^.*' . '(?:' . preg_replace('/\s/', '|',
...
function default_picture($data, $type = 'cv')
{
    ...
    return 'http://dummyimage.com/300x300/10588a/ffffff.jpg&text=' . strtoupper(substr($string1, 0,
                1) . substr($string2, 0, 1));
...
function default_color_picture($data, $type = 'cv')
{
    ...
    return [
        'color' => '#' . $color,
        'image' => 'http://dummyimage.com/300x300/' . $color . '/ffffff.jpg&text=' . $text
    ];
