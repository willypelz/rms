...
class SolrSearch implements SearchEngine
{
    ...
    {
        SolrPackage::$url = env("SOLR_URL");
        SolrPackage::$core = env("SOLR_CORE");
        SolrPackage::$host = env("SOLR_URL") . SolrPackage::$core . "/select?";
    }
    ...
    public function get_saved_cvs($data)
    {
        ...
        return SolrPackage::search_resume($data, $additional);
    }
    ...
    {
        $applicants = JobApplication::has('cv')->with('job', 'cv')->orderBy('id', 'asc')->chunk(100, function ($applicants) {
            foreach ($applicants as $applicant) {
                UploadApplicant::dispatch($applicant)->onQueue('solr');
            }
        });
    ...
    public function extend_network(
        $q, $type = "exp_company", $followers, $is_followers = true, 
        $row = 12, $start = 0, $default_op = 'AND', $start_year = '', $end_year = ''
    ) {
