 ...
 $(document).ready(function(){
  ...
  $field = $(this);
 	$('body #shortlistBtn').on('click',function(){
 		...
 		$.post("{{ route('mass-action') }}", {job_id: '{{ $appl->job->id }}',cv_ids :  cv_ids, app_ids: app_ids,status: 'WAITING' },function(data){

 				$( '#viewModal' ).modal('toggle');
        $.growl.notice({ message: "You have added " + $field.closest('.modal-body').find('.media-heading a').text() + " to the waiting list" });
            sh.reloadStatus();
        });
